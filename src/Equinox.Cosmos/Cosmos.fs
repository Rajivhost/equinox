...
type [<NoComparison>] ContainerStream = { container : Microsoft.Azure.Cosmos.Container; name: string }
...
module private DocDb =
    ...
    type Microsoft.Azure.Cosmos.Container with
        member container.TryReadItem(partitionKey : PartitionKey, documentId : string, ?options : ItemRequestOptions): Async<float * ReadResult<'T>> = async {
            ...
            try let! doc = async { return! container.ReadItemAsync(documentId, partitionKey, requestOptions = options, cancellationToken = ct) |> Async.AwaitTaskCorrect }
...
function sync(req, expectedVersion, maxEvents) {
    ...
    let private run (stream: ContainerStream) (expectedVersion: int64 option, req: Tip, maxEvents: int)
    ...
    let private logged (stream: ContainerStream) (expectedVersion, req: Tip, maxEvents) (log : ILogger)
        : Async<Result> = async {
        ...
        let! t, (ru,result) = run stream (expectedVersion, req, maxEvents) |> Stopwatch.Time
        let resultLog =
    ...
    let batch (log : ILogger) retryPolicy stream batch: Async<Result> =
        let call = logged stream batch
        Log.withLoggedRetries retryPolicy "writeAttempt" call log
    let mkBatch (stream: ContainerStream) (events: Equinox.Codec.IEvent<_>[]) unfolds: Tip =
    ...
    module Initialization =
        ...
        let init log (client: CosmosClient) (dbName,collName) mode skipStoredProc = async {
            ...
            let container = client.GetContainer(dbName,collName)
            if not skipStoredProc then
                do! createSyncStoredProcIfNotExists (Some log) container }
        let initAux (client: CosmosClient) (dbName,collName) rus = async {
...
module internal Tip =
    let private get (stream: ContainerStream, maybePos: Position option) =
        let ro = match maybePos with Some { etag=Some etag } -> ItemRequestOptions(IfNoneMatchEtag=etag) | _ -> null
        stream.container.TryReadItem(PartitionKey stream.name, Tip.WellKnownDocumentId, ro)
    let private loggedGet (get : ContainerStream * Position option -> Async<_>) (stream: ContainerStream, maybePos: Position option) (log: ILogger) = async {
    ...
    let tryLoad (log : ILogger) retryPolicy (stream: ContainerStream) (maybePos: Position option): Async<Result> = async {
 ...
 module internal Query =
    open FSharp.Control
    let private mkQuery (container : Container) maxItems (stream: ContainerStream) (direction: Direction) startPos : FeedIterator<Batch>=
        let query =
        ...
        let qro = new QueryRequestOptions(PartitionKey = Nullable(PartitionKey stream.name), MaxItemCount=Nullable maxItems)
    ...
    let private handleResponse direction (stream: ContainerStream) startPos (query: FeedIterator<Batch>) (log: ILogger)
    ...
    let walk<'event> (log : ILogger) container retryPolicy maxItems maxRequests direction (stream: ContainerStream) startPos
        ...
        let query = mkQuery container maxItems stream direction startPos
    ...
    let walkLazy<'event> (log : ILogger) client retryPolicy maxItems maxRequests direction (stream: ContainerStream) startPos
        (tryDecode : IIndexedEvent -> 'event option, isOrigin: 'event -> bool)
        : AsyncSeq<'event[]> = asyncSeq {
        let responseCount = ref 0
        let query = mkQuery client maxItems stream direction startPos
        let pullSlice = handleResponse direction stream startPos
        let retryingLoggingReadSlice query = Log.withLoggedRetries retryPolicy "readAttempt" (pullSlice query)
        let log = log |> Log.prop "batchSize" maxItems |> Log.prop "stream" stream.name
        let mutable ru = 0.
...
type [<NoComparison>] Token = { stream: ContainerStream; pos: Position }
module Token =
    let create stream pos : Equinox.Store.StreamToken = { value = box { stream = stream; pos = pos } }
    let (|Unpack|) (token: Equinox.Store.StreamToken) : ContainerStream*Position = let t = unbox<Token> token.value in t.stream,t.pos
...
type Connection(client: Microsoft.Azure.Cosmos.CosmosClient, [<O; D(null)>]?readRetryPolicy: IRetryPolicy, [<O; D(null)>]?writeRetryPolicy) =
...
type Gateway(conn : Connection, batching : BatchingPolicy) =
    ...
    member __.LoadBackwardsStopping log stream (tryDecode,isOrigin): Async<Store.StreamToken * 'event[]> = async {
        let! pos, events = Query.walk log stream.container conn.QueryRetryPolicy batching.MaxItems batching.MaxRequests Direction.Backward stream None (tryDecode,isOrigin)
    ...
    member __.Read log stream direction startPos (tryDecode,isOrigin) : Async<Store.StreamToken * 'event[]> = async {
        let! pos, events = Query.walk log stream.container conn.QueryRetryPolicy batching.MaxItems batching.MaxRequests direction stream startPos (tryDecode,isOrigin)
        return Token.create stream pos, events }
    member __.ReadLazy (batching: BatchingPolicy) log stream direction startPos (tryDecode,isOrigin) : AsyncSeq<'event[]> =
        Query.walkLazy log stream.container conn.QueryRetryPolicy batching.MaxItems batching.MaxRequests direction stream startPos (tryDecode,isOrigin)
    member __.LoadFromUnfoldsOrRollingSnapshots log (stream,maybePos) (tryDecode,isOrigin): Async<Store.StreamToken * 'event[]> = async {
        let! res = Tip.tryLoad log conn.TipRetryPolicy stream maybePos
        match res with
    ...
    member __.GetPosition(log, stream, ?pos): Async<Store.StreamToken> = async {
        let! res = Tip.tryLoad log conn.TipRetryPolicy stream pos
        match res with
    ...
    member __.LoadFromToken(log, (stream,pos), (tryDecode, isOrigin)): Async<LoadFromTokenResult<'event>> = async {
        let! res = Tip.tryLoad log conn.TipRetryPolicy stream (Some pos)
        match res with
    ...
    member __.Sync log stream (expectedVersion, batch: Tip): Async<InternalSyncResult> = async {
        ...
        let! wr = Sync.batch log conn.WriteRetryPolicy stream (expectedVersion,batch,batching.MaxItems)
        match wr with
...
module Caching =
    ...
    type CategoryTee<'event, 'state>(inner: Store.ICategory<'event, 'state, ContainerStream>, tee : string -> Store.StreamToken * 'state -> unit) =
        ...
        interface Store.ICategory<'event, 'state, ContainerStream> with
    ...
    let applyCacheUpdatesWithSlidingExpiration
            ...
            (category: Store.ICategory<'event, 'state, ContainerStream>)
            : Store.ICategory<'event, 'state, ContainerStream> =
...
type private Collection(container : Microsoft.Azure.Cosmos.Container, ?initContainer : Microsoft.Azure.Cosmos.Container -> Async<unit>) =
...
type Collections(categoryAndIdToDatabaseCollectionAndStream : string -> string -> string*string*string, [<O; D(null)>]?disableInitialization) =
    ...
    member internal __.Resolve(client : Microsoft.Azure.Cosmos.CosmosClient, categoryName, id, init) : ContainerStream * (unit -> Async<unit>) option =
        ...
        let coll = collections.GetOrAdd((databaseId,collectionId), fun (db,coll) -> Collection(client.GetContainer(db, coll), ?initContainer = init))
        { container = coll.Container; name = streamName },coll.InitializationGate
...
type Context(gateway: Gateway, collections: Collections, [<O; D(null)>] ?log) =
    ...
    member internal __.ResolveCollStream(categoryName, id) : ContainerStream * (unit -> Async<unit>) option =
...
type Resolver<'event, 'state>(context : Context, codec, fold, initial, caching, [<O; D(null)>]?access) =
    ...
    let category : Store.ICategory<_,_,ContainerStream> =
        match caching with
...
type Connector
    ...
    let clientOptions =
        ...
        | Some _ when cp.ConnectionMode = ConnectionMode.Direct -> invalidArg "gatewayModeMaxConnectionLimit" "Not admissible in Direct mode"
...
type Context
    ...
    member __.Sync(stream : ContainerStream, position, events: IEvent<_>[]) : Async<AppendResult<Position>> = async {
